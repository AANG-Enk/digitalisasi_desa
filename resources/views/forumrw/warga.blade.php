...
@section('title')
    ...
    @else
        Forum RW Warga - Digitalisasi Desa
    @endif
@endsection
...
<div class="d-flex justify-content-end">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active">@if (isset($user)) Forum RW {{ $user->name }} @else Forum RW Warga @endif</li>
        </ol>
    </nav>
</div>
@if(session('success'))
        ...
        <div class="user-profile-header d-flex flex-column flex-sm-row text-sm-start text-center mb-5">
          ...
          <div class="flex-grow-1 mt-4 mt-sm-12">
            ...
            <div class="d-flex gap-2 justify-content-end">
                <a href="{{ route('forumrw.index') }}" class="btn btn-info">
                    <i class="ri-focus-2-line ri-16px me-2"></i>Lihat Thread
                </a>
                <a href="{{ route('forum.saya') }}" class="btn btn-info">
                    <i class="ri-eye-line ri-16px me-2"></i>Thread Saya
                </a>
                @can('Forum RW Create')
                    <a href="{{ route('forumrw.create') }}" class="btn btn-primary">
                    ...
                    <div class="row mb-2 border-bottom align-items-center">
                        ...
                        <div class="col-md-6 col-12 order-md-2">
                            <div class="title mb-1">
                                <a href="{{ route('forumrw.show',$item->slug) }}">
                            ...
                            <div class="text mb-2">
                                ...
                                <div class="mt-2">
                                    ...
                                    <div class="d-flex justify-content-end gap-2">
                                        @if (isset($user))
                                            @can('Forum RW Update')
                                                <a href="{{ route('forumrw.edit',$item->id) }}" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Edit {{ $item->judul }}" class="text-secondary"><i class="menu-icon tf-icons ri-edit-2-line"></i></a>
                                            @endcan
                                            @can('Forum RW Delete')
                                                <form method="post" action="{{ route('forumrw.destroy',$item->id) }}" id="form-delete-{{ $loop->iteration }}" class="d-inline">
                                                    @csrf
                                                    @method('delete')
                                            ...
                                            @endcan
                                        @endif
                                        <a href="{{ route('forumrw.show',$item->slug) }}" class="btn btn-info btn-xs">Selengkapnya</a>
                                    </div>
                                </div>
                            </div>
                        </div>
